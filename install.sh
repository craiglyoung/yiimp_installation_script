...
#
# Program:
#   Install yiimp on Ubuntu 22.04 running Nginx, MariaDB, and PHP 7.4
#   v0.2-7.4 (update February, 2024)
# 
################################################################################

    script_version='v0.2-7.4'
    ...
    #Copy needed files
    sudo cp -r config/functions.sh /etc/
    sudo cp -r utils/screen-script.sh /etc/
    sudo cp -r config/editconf.py /usr/bin/
    ...
    echo -e "$GREEN Install yiimp on Ubuntu 22.04 running Nginx, MariaDB, and PHP 7.4 $COL_RESET"
    echo -e "$GREEN************************************************************************$COL_RESET"
    echo
    sleep 3
    ...
    sleep 3
    source config/getip.sh

    echo 'PUBLIC_IP='"${PUBLIC_IP}"'
    PUBLIC_IPV6='"${PUBLIC_IPV6}"'
    DISTRO='"${DISTRO}"'
    PRIVATE_IP='"${PRIVATE_IP}"'' | sudo -E tee config/pool.conf >/dev/null 2>&1
    ...
    # Installing Installing php 7.4
    echo
    echo
    echo -e "$CYAN => Installing php 7.4: $COL_RESET"
    echo
    sleep 3
    ...
    sleep 5
    hide_output sudo update-alternatives --set php /usr/bin/php7.4
    ...
    sudo chown ${whoami}:www-data /$HOME/
    sudo mkdir -p /$HOME/backup/
    sudo mkdir -p /$HOME/yiimp-logs/
    sudo chown -R ${whoami}:www-data /$HOME/yiimp-logs/
    ...
    define('"'"'YIIMP_MYSQLDUMP_PATH'"'"', '"'"''"/home/${whoami}/backup"''"'"');
    ...
    sudo sed -i "s|/root/backup|/home/${whoami}/backup|g" /var/web/yaamp/core/backend/system.php
